...
def create_ws_event_request_json(data):
    ...
    event_request[WS_DATA] = {WS_EVENT: data}
...
def validate_ws_event_message(msg_json, is_request, validate_success=True):
    ...
    event_msg_dict = msg_dict[WS_DATA]
    assert WS_EVENT in event_msg_dict, "'%s' missing" % WS_EVENT

    return validate_event_message(event_msg_dict[WS_EVENT],
